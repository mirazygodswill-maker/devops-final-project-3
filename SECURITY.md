...
- **Grafana is also publicly exposed** via its own `LoadBalancer` Service (`monitoring-grafana`), for convenience over keeping a `kubectl port-forward` tunnel open. This means the monitoring dashboards and admin login are reachable from the internet by anyone with the URL. Mitigated only by a non-default admin password — no IP allowlisting, WAF, or SSO/OAuth is in front of it. For production, this should sit behind a VPN, an IP-restricted security group, or an authenticating reverse proxy rather than being open to the public internet.
